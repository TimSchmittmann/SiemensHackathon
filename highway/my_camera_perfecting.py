...
class Projector:
    ...
    def colorize(self, index, points, size=4, view_distance=100,
        x_offset = 100, y_offset = -220,
        above_x_hide_self = 1700, below_y_hide_self = 2000):
        """
        Project given points into the image with given index. Returns rendered image
        ...
        for row in min_df.itertuples():
            if row.x > above_x_hide_self and row.y < below_y_hide_self:
                continue
            x_offset = 100
            y_offset = -220
...
# camera_pose 33 -> lidar section 0
# camera_pose 530 -> lidar section 16, 17
# camera_pose 1594 -> lidar section 48
# camera_pose 1778 -> lidar section 53, 54
...
camera_data_dir = ROOT_DIR / ".." / "highway_data" / "planar2_out"
...
planar_folder = "../highway_data/planar2_out"
out_folder = "../highway_data/out2"
proj = Projector(planar_folder)

# camera_pose_indices = list(range(530, 537))
camera_pose_idx = 33
pcds = []
for section_idx in range(0,1):
    print(section_idx)
    lidar_filename = f"reduced_{section_idx}_intensity.ply"
    ply_file = SPLITTED_DIR / lidar_filename
    points, colors = read_ply(ply_file)

    points_centered, pcd_centered = proj.colorize(
        camera_pose_idx, points, colors * 255, view_distance = 100,
        x_offset = 0, y_offset = 0,
        above_x_hide_self=1940 , below_y_hide_self=1750)
    pcds.append(pcd_centered)

    write_ply(f"{out_folder}/{camera_pose_idx}_output.ply", points_centered, colors)
...
camera_pose_idx = 530
pcds = []
for section_idx in range(16,18):
    print(section_idx)
    lidar_filename = f"reduced_{section_idx}_intensity.ply"
    ply_file = SPLITTED_DIR / lidar_filename
    points, colors = read_ply(ply_file)
    ...
    points_centered, pcd_centered = proj.colorize(
        camera_pose_idx, points, colors * 255, view_distance = 100,
        x_offset = 0, y_offset = 0,
        above_x_hide_self=1900 , below_y_hide_self=2350)
    pcds.append(pcd_centered)
...
camera_pose_idx = 1778
pcds = []
for section_idx in range(53,55):
    print(section_idx)
    lidar_filename = f"reduced_{section_idx}_intensity.ply"
    ply_file = SPLITTED_DIR / lidar_filename
    points, colors = read_ply(ply_file)
