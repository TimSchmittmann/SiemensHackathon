...
class Projector:
    ...
    def colorize(self, index, points, size=4, view_distance=50):
        ...
        # window_size = 5
        # px_ma = {}
        # for lx in range(self.camera_img_height):
        #     for ly in range(self.camera_img_width):
        #         current_key = (lx, ly)
        #         ma_keys = [
        #             (x, y) for x in range(lx, min(self.camera_img_height - 1, lx + window_size))
        #                 for  y in range(ly, min(self.camera_img_width - 1, ly + window_size))]
        #         px_ma[current_key] = sum(best_distances[key]["min"] for key in ma_keys) / window_size
